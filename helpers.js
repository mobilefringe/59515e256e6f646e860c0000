...
function getShopStores (all_categories) {
   // var all_categories = getStoreCategories();
    ...
    var shop_stores = [];
    $.each( all_categories , function( i, cat ) {
        if((cat.name.indexOf("Apparel") > -1) || (cat.name.indexOf("Accessories")> -1) || (cat.name.indexOf("Jewellery")> -1)|| (cat.name.indexOf("'s Wear")> -1)|| (cat.name.indexOf("'s Wear")> -1)|| (cat.name.indexOf("'s Wear")> -1)|| (cat.name.indexOf("'s Wear")> -1)) {Toys
            //console.log(cat);
            dine_categories_id.push(cat.id);
        }
    });
