...
function getDineStores (all_categories) {
   // var all_categories = getStoreCategories();
    ...
    $.each( all_categories , function( i, cat ) {
        if((cat.name.indexOf("Food") > -1) || (cat.name.indexOf("Restaurant")> -1) || (cat.name.indexOf("Cafe")> -1)) {
            //console.log(cat);
...
function getPlayStores (all_categories) {
   // var all_categories = getStoreCategories();
    ...
    var play_stores = [];
    $.each( all_categories , function( i, cat ) {
        if((cat.name.indexOf("Toys") > -1) || (cat.name.indexOf("ATTRACTIONS")> -1) || (cat.name.indexOf("Gallery")> -1)){
            //console.log(cat);
